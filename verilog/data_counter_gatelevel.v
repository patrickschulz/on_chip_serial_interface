module data_counter(clk, reset, data_ready);
    ...
    and_gate _28_ (.A(net1[0]), .B(high[0]), .O(data_ready2[0]));
    and_gate _29_ (.A(net1[1]), .B(high[1]), .O(data_ready2[1]));
    and_gate _30_ (.A(net1[2]), .B(high[2]), .O(data_ready2[2]));

    and_gate _31_ (.A(data_ready2[0]), .B(data_ready2[1]), .O(data_ready1)); 
    and_gate _32_ (.A(data_ready1), .B(data_ready2[2]), .O(data_ready)); 
